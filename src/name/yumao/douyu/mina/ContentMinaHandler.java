...
//		}
		...
		if(sttDecoder.GetItem("type").equals("donateres")){
			...
			if(filepath!=null&&(!filepath.equals(""))){
				...
				if(filepath!=null&&(!filepath.equals(""))){}else{
					tti.setToolTip(snick + " 赠送给主播" + ms + "个鱼丸", Color.RED);
		...
		if(sttDecoder.GetItem("type").equals("bc_buy_deserve")){
			...
			suiDecoder.Parse(sui);
			String snick = suiDecoder.GetItem("nick");
			logger.debug("接收到酬勤消息\t" + snick + " 赠送给主播" + count + "个" + levStr[Integer.parseInt(lev)]);
			if(filepath!=null&&(!filepath.equals(""))){
				//进行弹幕分析 拉起字幕转储进程
				...
				if(filepath!=null&&(!filepath.equals(""))){}else{
					...
					tti.setToolTip(snick + " 赠送给主播" + count + "个" + levStr[Integer.parseInt(lev)], Color.RED );
				}
			}
		}
